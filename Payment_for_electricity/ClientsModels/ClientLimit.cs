...
namespace Payment_for_electricity.ClientsModels
{
    ...
    {
        public ClientLimit(string name, string address, Tariff tariff, double energyVolume) : base(name, address, tariff, energyVolume)
        {
            Type = ClientType.ClientLimit;
        }
        protected override void SetTariff(Tariff t)
        {
            Type typeThis = GetType();
            Type typeTariff = t.GetType();
            if (typeTariff.Name == TariffType.TariffLimit.ToString())
                tariff = t;
            else
                throw new Exception("Клиенту " + Name + " с типом клиента " + typeThis.Name + " невозможно установить тариф " + typeTariff.Name +
                    ".\nТариф должен быть " + TariffType.TariffLimit.ToString());
        }
    }
}
